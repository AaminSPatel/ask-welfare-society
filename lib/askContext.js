...
export const AskProvider = ({ children }) => {
  ...
  // Load user from localStorage on app start and set token immediately
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (storedUser) {
        try {
          ...
          if (userData.expiry && Date.now() < userData.expiry && userData.token) {
            ...
            api.defaults.headers.common['Authorization'] = `Bearer ${userData.token}`;
            setUser(userData);
            console.log('✅ User loaded and token set in axios headers');
          } else {
            console.log('❌ User token expired or invalid, clearing localStorage');
            localStorage.removeItem('user');
            delete api.defaults.headers.common['Authorization'];
          }
        } catch (error) {
